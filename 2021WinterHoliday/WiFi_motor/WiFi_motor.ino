...
//设置步进电机的步数和引脚（就是注意点2里面说的驱动板上IN1～IN4连接的四个数字口）. 引脚要互换
...
void serial_check(){
    ...
    if (comdata.length() > 0)
    {
        ...
        int index_command_begin = 0; int index_command_end = 0;
        for (int i = 0; i<=comdata.length();i++){
          if (comdata[i]=='#') {
            index_command_begin = i;
            break;
          }
          if (comdata[i]=='&') {
            index_command_end = i;
        ...
        stime = comdata.substring(index_command_begin+1,index-index_command_begin);
